...
def tratar_xml(uploaded_file):
    ...
    def coletar_codigo_jbs(df):
        ...
        for i, info in enumerate(df.get('infAdProd', [])):
            encontrado = False
            for padrao, extracao in padroes.items():
                if padrao in str(info):
                    try:
                        if df['prod.CFOP'][i] == '5125':
                            info = '836289'
    ...
    novas_colunas = {
        ...
        'Emitente Nome': emit.get('xNome').upper(),
        'Destinatário CNPJ': dest.get('CNPJ'),
        'Destinatário Nome': dest.get('xNome').upper(),
    }
    ...
    # Reorganiza colunas
    colunas_ordenadas = list(novas_colunas.keys()) + \
    ...
    df = df[['Série', 'NF', 'Operação', 'Emitente Nome', 'Emitente CNPJ',
             'Destinatário Nome', 'Destinatário CNPJ',
             'Código JBS', 'prod.xProd', 'prod.CFOP', 'prod.uCom', 'prod.qCom', 'prod.vUnCom', 'prod.vProd']]

    df.columns = ['Série', 'NF', 'Operação', 'Emitente Nome', 'Emitente CNPJ',
                  'Destinatário Nome', 'Destinatário CNPJ',
                  'Código JBS', 'Produto', 'CFOP', 'Unidade de Medida', 'Qtde', 'Valor Unitário', 'Valor Total']
    ...
    df['Valor Unitário'] = df['Valor Unitário'].astype(
        'float', errors='ignore')
...
def concatenar_df(uploaded_files):
    ...
    todos_df = [tratar_xml(uploaded_file.read())
